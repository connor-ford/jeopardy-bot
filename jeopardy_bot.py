...
print('-----')
print('Jeopardy Bot: a Twitter bot that automatically posts Jeopardy! questions, then answers them.')
...
while True:
    ...
    time_last = datetime.strptime(last_tweet['created_at'], '%a %b %d %H:%M:%S %z %Y').replace(tzinfo=None)
    ...
    if not last_tweet['in_reply_to_status_id']:
        ...
        if t_response is not None:
            print('Success.')
            print('Successfully Retweeted Tweet %s, id %s' %(last_tweet['id_str'], t_response['id_str']))
        else:
            print('Failed.')
        continue
